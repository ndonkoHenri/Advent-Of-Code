...
for x in instructions:
    ...
    # get the values to be removed from the list in exact/same order
    moved_values = my_parser[move_from][(-1 * how_many):]
    # remove them from the old list
    my_parser[move_from] = [a for a in my_parser[move_from][:(-1 * how_many)]]
    # send them to the new list
    my_parser[move_to].extend(moved_values)
